...
const FS_SPLAT = `precision highp float;
  ...
  void main() {
    vec2 p = vUv - uPointer;
    p.x *= aspectRatio;
    vec2 velDir = length(uColor.xy) > 0.0001
      ? normalize(uColor.xy)
      : vec2(1.0, 0.0);
    float along = dot(p, velDir);
    float perp  = length(p - along * velDir);
    float stretch = 1.0;
    float d = (along * along) / (uRadius * stretch) + (perp * perp) / uRadius;
    vec3 splat = exp(-d) * uColor;
...
const FS_DISPLAY = /* glsl */`
  ...
  void main() {
    ...
    gl_FragColor = vec4(mix(uBgColor, text.rgb, text.a), 1.0);
  }
`;

// ─── FluidText CLASS ──────────────────────────────────────────────────────────

export class FluidText {
  ...
  _init() {
    ...
    const bg = new THREE.Color(cfg.BG);
    this.mDisplay = mk(FS_DISPLAY);
    this.mDisplay.uniforms = { tVelocity:{value:null},
  ...
  tick(dt) {
    ...
    if (!this._movedThisFrame && this._hasPos) {
      ...
      if (mag > 40) {
        this._splat(
          this._lastPos.x, this._lastPos.y,
          this._momentum.x * 0.4,
          this._momentum.y * 0.4
        );
